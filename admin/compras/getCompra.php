...
if ($orden == null) {
    echo json_encode('<p>Orden no válida.</p>', JSON_UNESCAPED_UNICODE);
    exit;
}
...
if (!$rowCompra) {
    echo json_encode('<p>Compra no encontrada.</p>', JSON_UNESCAPED_UNICODE);
    exit;
}
...
$sqlDetalle = $con->prepare("SELECT nombre, precio, cantidad 
                             FROM detalle_compra 
                             WHERE id_compra = ?");
...
$html .= '<thead><tr><th>Producto</th><th>Precio</th><th>Cantidad</th><th>Subtotal</th></tr></thead>';
$html .= '<tbody>';
