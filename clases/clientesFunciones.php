...
function registraUsurario(array $datos, $con){
 ...
 if($sql->fetchColumn() > 0){
    ...
    $msg = "No existe los datos del cliente.";
 ...
 function activarUsuario($id, $con) {
    $sql = $con->prepare("UPDATE usuarios SET activacion = 1, token = '' WHERE id = ?"); // Cambiamos "usuario" a "usuarios"
...
function actualizaPassword($user_id, $passwordHash, $con) {
    $sql = $con->prepare("UPDATE usuarios SET password = ?, token_password = NULL, password_request = 0 WHERE id = ?");
    $resultado = $sql->execute([$passwordHash, $user_id]);
    
    if ($resultado) { // Verifica si la actualización fue exitosa
        return true;
    } else {
        return false;
    }
